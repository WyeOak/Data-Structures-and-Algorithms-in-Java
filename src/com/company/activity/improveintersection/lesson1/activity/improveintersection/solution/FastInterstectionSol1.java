package com.company.activity.improveintersection.lesson1.activity.improveintersection.solution;


import com.company.activity.improveintersection.lesson1.binarysearch.BinarySearch;
...
public class FastInterstectionSol1 {
    ...
    public List<Integer> intersectionFast(int[] a, int[] b) {
        ...
        // перед поиском сортируем массив
        mergeSort(b);
        // сравниваем каждый элемент
        for (int x : a) {
            if (search.binarySearch(x, b)){
                result.add(x);
            }
        }
        return result;
